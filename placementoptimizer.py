...
def parse_args():
    ...
    showsp = sp.add_parser('show', parents=[statep, upmapp])
...
class PGMappings:
    ...
    def __init__(self,
        ...
        if add_upmaps or ignore_cluster_upmaps:
    ...
    def revert_upmaps(self):
        """
        un-do all adjustments from the default crush-mapping state.
        """

        for pgid, pg_upmaps in self.cluster.upmap_items.items():
            for osd_from, osd_to in pg_upmaps:
                move_ok = self.move_pg(pgid, osd_to, osd_from)
                if not move_ok:
                    raise Exception(f'failed reverting move of {pgid} from {osd_to} '
                                    f'this would be due to an internal inconsistency: '
                                    f'the cluster upmap items indicate the pg is on {osd_to}, '
            ...
            for upmap_line in upmap_file:
                ...
                if items_match:
                    ...
                    for osd_from, osd_to in moves:
                        ...
                        if not move_ok:
                            logging.info(strlazy(lambda: f"skipped moving pg={pgid} from {osd_from}->{osd_to} since it's not mapped on osd.{osd_from}"))

                else:
                    raise Exception(f'unknown line content {upmap_line}')

    def move_pg(self, pgid, osd_from, osd_to) -> bool:
        """
    ...
    def apply_remap(self, pgid, osd_from, osd_to) -> bool:
        """
        simulate a remap pg from one osd to another.
    ...
    def get_pg_upmap(self, pgid):
        ...
        # remaps_list is [(osdfrom, osdto), ...],
        # convert it to osdfrom->osdto
