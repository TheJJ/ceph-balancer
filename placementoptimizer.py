...
def parse_args():
    ...
    statep.add_argument("--state", "-s", help="load cluster state from a JSON file")

    cephcmdp = argparse.ArgumentParser(add_help=False)
    cephcmdp.add_argument("--ceph-command", default="ceph",
                          help="command to invoke when communicating with Ceph. default: %(default)s")

    osdsizep = argparse.ArgumentParser(add_help=False)
    osdsizep.add_argument('--osdsize', choices=['device', 'weighted', 'crush'], default="crush",
                          help=("the method for determining OSD size. default: %(default)s. "
                                "device=device_size, weighted=devsize*weight, crush=crushweight*weight"
                                "where device is the reported size of the block device,"
                                "      weighted is the block device's size multiplied by the legacy REWEIGHT value,"
                                "      crush is the CRUSH weight, which in rare situations varies from the block device size."))
    ...
    upmapignorep.add_argument('--ignore-state-upmaps', action='store_true',
                              help='pretend the cluster has no existing upmaps')
    ...
    upmapp.add_argument('--add-upmaps',
                        help=('after loading cluster state, simulate application of PG movements '
                              'from a plan generated by "balance"'))
    ...
    predictionp.add_argument('--avail-prediction', choices=['weight', 'limiting'], default='limiting',
                             help=("algorithm to use for available pool size prediction. default: %(default)s\n"
                                   ...
                                   "This unfortunately may be wrong for multi-take CRUSH rules, "
                                   "doesn't respect the current PG placements.\n"
                                   "limiting: determine space by finding the pool's limiting (most-full) OSD and extrapolate possible usage."))
    ...
    usedestimatep.add_argument('--osdused', choices=["delta", "shardsum"], default="shardsum",
                               help=('how is OSD fullness predicted during simulation? default: %(default)s. '
                                     "delta: adjust the built-in OSD fullness report by in-move PG deltas - more accurate but doesn't account pending data deletion.\n"
                                     "shardsum: estimate the usage by summing up all PG shard sizes - doesn't account PG metadata overhead."))

    savemappingp = argparse.ArgumentParser(add_help=False)
    savemappingp.add_argument("--save-mappings", help="filename to store the resulting up set for all PGs (to see where things are placed)")

    ### subcommands
    gathersp = sp.add_parser('gather', parents=[cephcmdp],
                             help="only gather cluster information, i.e. generate a state file")
    gathersp.add_argument("output_file", help="file to which to write cluster balancing information")
    ...
    showsp.add_argument('--only-crushclass',
                        help="only display devices of this CRUSH device class")
    showsp.add_argument('--sort-shardsize', action='store_true',
                        help="sort the pool overview by shard size")
    showsp.add_argument('--osds', action='store_true',
                        help="show info about all OSDs instead of just the pool overview")
    ...
    showsp.add_argument('--normalize-pg-count', action='store_true',
                        help="normalize PG count by device size")
    ...
    remap_showp.add_argument('--osds',
                             help="only consider these OSD when using --by-osd, space separated")
    ...
    remap_undop.add_argument('--output', '-o', default="-",
                             help="output filename for resulting movement instructions. default is stdout.")
    remap_undop.add_argument('--exclude-backfilling', action="store_true",
                             help="only take backfill_wait PGs, exclude currently backfilling PGs")
    ...
    _remap_drainp = remapsp.add_parser('drain', parents=[cephcmdp, statep, osdsizep],
                                       help=("generate upmap items to empty OSDs. "
                                             "consider setting an OSD's CRUSH weight to 0 instead."))

    balancep = sp.add_parser('balance', parents=[cephcmdp, statep, upmapp, osdsizep, usedestimatep, savemappingp],
                             help="distribute PGs for better capacity and performance")
    ...
    balancep.add_argument('--only-crushclass',
                          help='comma separated list of CRUSH device classes to balance')
    balancep.add_argument('--source-osds',
                          help=("only consider these OSDs as movement sources, separated by ',' or ' '. "
    ...
    balancep.add_argument('--osdfrom', choices=["fullest", "limiting", "alternate"], default="alternate",
                          help=('how to determine the source OSD for a movement? default: %(default)s. '
                                "fullest=start with the most full OSD (by percent of device size). "
                                "limiting=start with the most full OSD that actually limits the usable pool space. "
                                "alternate=alternate between limiting and most-full devices"))
    ...
    pooldiffp.add_argument('--pgstate', choices=['up', 'acting'], default="acting",
                           help="which PG set to take, up or acting (default acting).")
    pooldiffp.add_argument('pool1',
                           help="use this pool for identifying involved OSDs")
    pooldiffp.add_argument('pool2',
                           help="compare to this pool which in which OSDs are involved")
