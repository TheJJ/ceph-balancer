...
class ClusterState:
    ...
    def export_osdmap(self, output_file):
        ...
        # max_osd: s32
        writeb(struct.pack('<I', self.state['osd_dump']['max_osd']), 'max_osd')

        # osd_state: vector<uint32>
        writeb(struct.pack('<I', len(self.state['osd_dump']['osds'])), 'osd_state_len')
        state_bits = {
            "exists": 1 << 0,
		    "up": 1 << 1,
		    "autoout": 1 << 2,
        ...
        for osdmetadata in self.state['osd_dump']['osds']:
            ...
            writeb(struct.pack('<I', osd_state), f'osd_state_{osdmetadata["osd"]}')

        # osd_weight: vector<uint32>
        writeb(struct.pack('<I', len(self.state['osd_dump']['osds'])), 'osd_weight_len')
        for osdmetadata in self.state['osd_dump']['osds']:
            # the osd dump uses floats...
            weight = (osdmetadata['weight'] * 0x10000) // 1
            weight_b = struct.pack('<I', weight)
            writeb(weight_b, f'osd_weight_{osdmetadata["osd"]}:{weight}')

        # osd_addrs->client_addrs: vector<sharedptr<entity_addrvec_t>>
        writeb(struct.pack('<I', len(self.state['osd_dump']['osds'])), 'osd_addrs')
        for osdmetadata in self.state['osd_dump']['osds']:
            public_addrs = osdmetadata['public_addrs']['addrvec']
            # entity_addrvec_t::encode
            writeb(struct.pack('<B', 2), f'osd_addr_{osdmetadata["osd"]}_v')
            writeb(struct.pack('<I', len(public_addrs)), f'osd_addr_{osdmetadata["osd"]}_len')
            for addr in public_addrs:
                ...
                writeb(struct.pack('<I', addr_type[addr['type']]), f'osd_addr_{osdmetadata["osd"]}_type')
                # nonce: u32
                writeb(struct.pack('<I', addr['nonce']), f'osd_addr_{osdmetadata["osd"]}_nonce')

                # elen: u32, ss_family: u16,
                writeb(encode_ipstr(addr['addr']), f'osd_addr_{osdmetadata["osd"]}_addr')
